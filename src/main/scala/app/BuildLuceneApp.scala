      ...
      Pdfs.readText(pdf).fold(
        ...
        text => {
          ...
          chunks.grouped(batchSize).zipWithIndex.foreach { case (group, gi) =>
            val vecs = group.map(chunk => client.embed(chunk, opts.model))
...
@main def buildIndex(): Unit =
  val pdfDir = "C:\\Users\\HP\\IdeaProjects\\FirstScala\\MSRCCorpus"
  val pdfs = Vector(
    "1083142.1083143.pdf",
    "1083142.1083144.pdf"
  ).map(name => java.nio.file.Paths.get(pdfDir, name))

  val indexPath = java.nio.file.Paths.get("lucene-index")
  ...
  pdfs.foreach { pdf =>
    val docId = pdf.getFileName.toString.replaceAll("\\.pdf$", "")
    println(s"\n📘 Processing: $docId")

    util.Pdfs.readText(pdf) match
      case scala.util.Success(text) =>
        val chunks = util.Chunker.split(text, maxChars = 1200, overlap = 200)
        println(s"✅ ${text.length} chars → ${chunks.size} chunks")

        chunks.zipWithIndex.foreach { case (chunk, i) =>
          val emb = ollama.embed(chunk, model)
          index.LuceneIndex.add(
            indexDir = indexPath,
            docId    = docId,
            chunkId  = i,
            text     = chunk,
            vec      = emb,
            pathRel  = s"$docId#chunk_$i"
          )
          println(f"  indexed chunk $i%3d (dim=${emb.length})")
        }
      case scala.util.Failure(ex) =>
        System.err.println(s"❌ Failed: ${ex.getMessage}")
  }

  println(s"\n✅ Done. Index stored at: ${indexPath.toAbsolutePath}")
